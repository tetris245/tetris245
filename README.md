...
* New command: lock to add locks on all lockable items, on yourself or another player. The lock type is specified by a number between 1 and 9: 1 Metal - 2 Exclusive - 3 Intricate - 4 High Security - 5 Pandora - 6 Mistress - 7 Lover - 8 Owner -  9 Five Minutes. The r parameter can be used with lock type 9 to allow automatic removal of the item when the lock timer runs out. 
